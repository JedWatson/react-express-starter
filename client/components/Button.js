...
module.exports = React.createClass({
	...
	render() {
		...
		function buttonVariant(background, color) {
			var styles = {
				backgroundImage: cssVerticalGradient(Colours.shadeColor(background, 0.04), Colours.shadeColor(background, -0.04)),
			...
			if (self.state.hover) {
				styles = _.extend(styles, {
					backgroundImage: cssVerticalGradient(Colours.shadeColor(background, 0.08), background),
					outline: 'none'
				});
			}
			if (self.state.focus) {
				styles = _.extend(styles, {
					backgroundImage: cssVerticalGradient(Colours.shadeColor(background, 0.08), background),
					boxShadow: '0 0 0 1px ' + self.ui.color.bodyBackground + ', 0 0 0 3px ' + Colours.blendColors(self.ui.color.bodyBackground, background, 0.3),
					outline: 'none'
				});
			}
			if (self.state.active) {
				styles = _.extend(styles, {
					backgroundColor: Colours.shadeColor(background, -0.04),
		...
		function buttonDefaultVariant(border, color) {
			...
			if (self.state.focus || self.state.hover) {
				styles = _.extend(styles, {
					backgroundColor: Colours.blendColors(color, self.ui.color.bodyBackground, 0.9),
					backgroundImage: 'none',
					borderColor: Colours.shadeColor(border, 0.1),
					outline: 'none'
				});
			}
			if (self.state.focus) {
				styles = _.extend(styles, {
					boxShadow: '0 0 0 1px ' + self.ui.color.bodyBackground + ', 0 0 0 3px ' + Colours.blendColors(self.ui.color.bodyBackground, border, 0.3),
				});
			}
			if (self.state.active) {
				styles = _.extend(styles, {
					backgroundColor: Colours.blendColors(color, self.ui.color.bodyBackground, 0.8),
					borderColor: Colours.shadeColor(border, -0.1),
					boxShadow: 'none',
					color: color
				});
		...
		function buttonLink(color) {
			...
			if (self.state.hover) {
				styles = _.extend(styles, {
					color: Colours.shadeColor(color, 0.1),
